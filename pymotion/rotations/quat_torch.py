...
def weighted_slerp(
    ...
    for i in range(1, num_items):
        weight_i = weights[i]
        if weight_i > 1e-8:
            t = weight_i / (accumulated_weight + weight_i)
            result = slerp(result, quaternions[i], t, shortest=shortest)
            accumulated_weight = accumulated_weight + weight_i
...
def scale(quaternions: torch.Tensor, factor: float | torch.Tensor) -> torch.Tensor:
    ...
    # Convert to scaled angle-axis, scale, and convert back
    scaled_axis = to_scaled_angle_axis(quaternions) * factor

...
def fk_quat(
    ...
    global_rotations[..., 0, :] = rot[..., 0, :]

    # Compute forward kinematics for all other joints
...
def fk(
    ...
    if rot.shape[-1] == 4:
        ...
        raise ValueError(
            "rot must have shape (..., n_joints, 4) or (..., n_joints, 3, 3)"
        )
    mat[..., :3, 3] = offsets
    mat[..., 3, 3] = 1
    ...
    for j, children_of_j in enumerate(children):
        ...
        for gc in children_of_j[1:]:
            ...
            roll_axis = quat.mul_vec(
                quat.inverse(global_rots[:, j]),
                vec.normalize(positions[:, c] - positions[:, j]),
            )
...
def mirror(
    ...
    if mode == "all":
        ...
        if len(joints_mapping) != len(parents):
            raise ValueError(
                "joints_mapping must have the same length as the number of joints"
            )
        if axis == "X":
            mirror_index = 0
            q_mirror = (2, 3)
